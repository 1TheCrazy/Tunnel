...
pub async fn register_self(self_server: &mut Node) {
    if self_server.self_id.is_empty() {
        ...
        let json: CreateNodeResponse = match register_req_res.json().await {
            Ok(json) => json,
            Err(err) => {
                println!("node: self registration response decode failed error={}", err);
                panic!("Wasn't able to register self: {}", err)
            }
        };

        self_server.self_id = json.assigned_id;
        println!("node: self registration succeeded assigned_id={}", self_server.self_id);

    } else {
        println!("node: already registered assigned_id={}", self_server.self_id);
    }
}
